...
gulp.task(`server`, (done)=>{
    exec(`pm2 start ./src/app.js --name getImages`, function (err, stdout, stderr) {
        console.log(err, stdout, stderr)
        done()
    })
})
...
gulp.task(`server-reload`, (done) => {
    exec(`pm2 restart getImages`, function (err, stdout, stderr) {
        console.log(err, stdout, stderr)
        done()
    })
})
...
gulp.task(`dev-build`, (done)=>{
    gulpSequence(`set-dev-env`, `server`)(done)
})

gulp.task(`dev-reload`, (done) => {
    gulpSequence(`set-dev-env`, `server-reload`)(done)
})
